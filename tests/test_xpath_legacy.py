# TODO transfer applicable tests to the new test module
...
from _delb.xpath import (
    ...
    LegacyFunctionExpression,
    LegacyLiteral,
    LegacyPredicateExpression,
...
def test_function():
    ...
    assert isinstance(predicates.arguments[0], LegacyAttributePredicate)
    assert predicates.arguments[0].name == "foo"

    assert isinstance(predicates.arguments[1], LegacyLiteral)
...
def test_function_args():
    parsed_expression = LegacyXPathExpression(
...
def test_multiple_predicates():
    ...
    assert isinstance(predicates, LegacyBooleanPredicate)
    assert predicates.operator == "and"
    assert isinstance(predicates.left_operand, LegacyAttributePredicate)
    assert isinstance(predicates.right_operand, LegacyAttributePredicate)
    assert (
...
def test_predicate_order():
    parsed_expression = LegacyXPathExpression("./node[@foo and (@this or @that)]")
    predicates = parsed_expression.location_paths[0].location_steps[1].predicates
    ...
    assert isinstance(left_operand, LegacyAttributePredicate)
    assert left_operand.name == "foo"
    assert isinstance(right_operand, LegacyBooleanPredicate)
    ...
    assert isinstance(right_operand, LegacyAttributePredicate)
...
def test_semantic_consistency():
    assert (
        str(LegacyXPathExpression('.//pb[@n="I"]'))
